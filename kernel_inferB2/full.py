...
def _surf_one(hw, wid, k_wells, k_pts, max_dev=40.0):
    C_ = _surf_cloud()
    if not C_:
        return None
    ev_idx = hw.index[hw["TVT_input"].isna()]
    ...
    dip = a_ * rx + c_ * ry - rz
    return dict(ev_idx=ev_idx, dev=dev, dip=dip, dx=dx_, a=a_, c=c_, nw=nw)


SURF_SCALES = (8, 20, 45)


def surf_rows_for_well(wid, split, k_pts=240):
    """Multi-scale structural surface: a tight, a medium and a regional
    neighbourhood. Their disagreement is the confidence signal -- where the
    scales agree the structure is trustworthy, where they diverge it is not."""
    hw, _tw = load_well(wid, split=split)
    outs = {}
    for k in SURF_SCALES:
        r = _surf_one(hw, wid, k, k_pts)
        if r is not None:
            outs[k] = r
    if not outs:
        return None
    ref = outs[max(outs)]
    d = {"id": [f"{wid}_{i}" for i in ref["ev_idx"]]}
    devs = []
    for k in SURF_SCALES:
        r = outs.get(k)
        if r is None or len(r["dev"]) != len(ref["dev"]):
            continue
        devs.append(r["dev"])
        d[f"surf_dev{k}"] = r["dev"].astype(np.float32)
        d[f"surf_dipdx{k}"] = (r["dip"] * r["dx"]).astype(np.float32)
        d[f"surf_dip{k}"] = np.float32(r["dip"])
    if not devs:
        return None
    D = np.vstack(devs)
    d["surf_dev"] = D.mean(axis=0).astype(np.float32)
    d["surf_disagree"] = D.std(axis=0).astype(np.float32)
    d["surf_a"] = np.float32(ref["a"]); d["surf_c"] = np.float32(ref["c"])
    d["surf_nw"] = np.float32(ref["nw"])
    return pd.DataFrame(d)
...
# Post-process weights re-fitted on this model's own cross-fitted OOF with
# GroupKFold over wells (tune_pp2.py). The prediction is linear in these weights
# and the smoother is a linear operator, so they are solved in closed form rather
# than hand-tuned. Honest pooled CV: 9.1921 (author defaults) -> 9.1398.
PP_W = {"model": 0.5797, "likpf_scale_3": 0.3278,
        "surf_dev8": 0.0468, "surf_dev20": 0.0467, "surf_dev45": 0.0070}


def make_prediction_B(df, model_delta):
    last = df["last_known_tvt"].values.astype(float)
    warm = warmup_B(df["md_since"].values.astype(float), PP_B.tau)
    delta = PP_W["model"] * warm * model_delta
    for col, w in PP_W.items():
        if col == "model" or col not in df.columns:
            continue
        v = df[col].values.astype(float)
        delta = delta + w * (v - last if col.startswith("likpf") else v)
    pred = last + delta
    out = pred.copy()
...
def main_B():
    ...
    if models_dir is not None:
        ...
        # Dump everything make_prediction_B consumes, so the post-processing
        # (warmup tau, blend weights, smoothing) can be retuned offline against
        # this same pooled CV without paying for another full retrain.
        _dump_cols = ["id", "well", "last_known_tvt", "md_since", "pf_ancc", "target"]
        _dump_cols += [c for c in train_df_B.columns if c.startswith("likpf_")]
        _dump_cols += [c for c in train_df_B.columns if c.startswith("surf_")]
        _oof = train_df_B[[c for c in _dump_cols if c in train_df_B.columns]].copy()
        _oof["meta_oof"] = meta_oof
        _oof.to_pickle(CFG.OUT / "oof_B.pkl")
        print(f"saved oof_B.pkl ({_oof.shape}) for offline post-process tuning")
