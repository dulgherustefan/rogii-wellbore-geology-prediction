...
def main_B():
    ...
    if models_dir is not None:
        print(f"INFERENCE mode — loading native Pipeline-B stack(s) from {models_dir}")
        ...
        # Blend in any additional mounted stacks. Their feature lists are subsets
        # of what we compute, so each stack is fed exactly the columns it was
        # trained on. Averaging two independently-trained stacks hedges the risk
        # that the newer, more spatial one does not transfer to unseen wells.
        _extra_dirs = []
        for _r in _infer_roots:
            _r = Path(_r)
            if _r.exists() and (_r / "manifest.json").exists() and _r != Path(models_dir):
                _extra_dirs.append(_r)
        _preds = [meta_test]
        for _d in _extra_dirs:
            try:
                _mn = json.load(open(_d / "manifest.json"))
                if not set(_mn["feats"]).issubset(set(test_df_B.columns)):
                    print(f"  skip {_d}: needs features we do not compute")
                    continue
                _Xt = test_df_B[_mn["feats"]].values.astype(np.float32)
                _cc = []
                for _n in _mn["col_order"]:
                    if _n.startswith("lgb"):
                        _cc.append(_lgbm.Booster(model_file=str(_d / f"{_n}.txt")).predict(_Xt))
                    else:
                        _m2 = CatBoostRegressor(); _m2.load_model(str(_d / f"{_n}.cbm"))
                        _cc.append(_m2.predict(_Xt))
                _preds.append(np.column_stack(_cc) @ np.array(_mn["ridge_coef"], float)
                              + float(_mn["ridge_intercept"]))
                print(f"  blended stack from {_d} ({len(_mn['feats'])} feats)")
            except Exception as _e:
                print(f"  skip {_d}: {_e}")
        if len(_preds) > 1:
            meta_test = np.mean(_preds, axis=0)
            print(f"BLEND of {len(_preds)} stacks")
