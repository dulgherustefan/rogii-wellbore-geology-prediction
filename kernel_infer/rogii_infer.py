...
def main():
    ...
    obj = joblib.load(_find_model()); models = obj['models']; FE = obj['feats']
    print('building cloud...', flush=True); ensure_cloud()
    ...
    te = [x for x in Parallel(n_jobs=NJOBS, backend='threading', verbose=5)(delayed(well_feats)(w, 'test') for w in test_wids) if x is not None]
    out = {}
    for fte in te:
        wid = fte['wid'].iloc[0]; anchor = fte['anchor'].iloc[0]
        pr = np.mean([m.predict(fte[FE].values) for m in models], axis=0)
        pr = np.clip(pr, -90, 90)
