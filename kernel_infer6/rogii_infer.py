...
def pf_ensemble(hw, tw, n_seeds=40, own_ref=False, scale=5.0, **kw):
    ...
    for s in range(n_seeds):
        p, ll = run_pf(hw, tt, tg, seed=s, **kw); preds.append(p); liks.append(ll)
...
def ensure_cloud():
    ...
    if 'tree' not in CL:
        ...
        # per-well centroid cloud for plane fitting (one point per well)
        uw = np.unique(WID); CXY = np.zeros((len(uw), 2)); CEE = np.full((len(uw), 6), np.nan)
        for i, w in enumerate(uw):
            m = WID == w; CXY[i] = np.nanmedian(XY[m], axis=0); CEE[i] = np.nanmedian(EE[m], axis=0)
        CSCALE = np.where(CXY.std(0) < 1e-3, 1.0, CXY.std(0))
        CL.update(CWID=uw, CXY=CXY, CEE=CEE, CSCALE=CSCALE, CTREE=cKDTree(CXY/CSCALE))
    return CL

def struct_plane_pred(h, exclude=None, k=10, decay=0.02):
    c = ensure_cloud(); CWID = c['CWID']; CXY = c['CXY']; CEE = c['CEE']; CSCALE = c['CSCALE']; CTREE = c['CTREE']
    ti = h['TVT_input'].values.astype(float); Z = h['Z'].values.astype(float); kn = np.isfinite(ti)
    anchor = ti[kn][-1] if kn.any() else 0.0
    q = h[['X', 'Y']].values.astype(float)
    nf = min(k+6, len(CWID)); d, ii = CTREE.query(q/CSCALE, k=nf)
    Ep = np.full((len(q), 6), np.nan)
    for j in range(len(q)):
        idx = ii[j]; dd = d[j]
        if exclude is not None:
            m = CWID[idx] != exclude; idx = idx[m]; dd = dd[m]
        idx = idx[:k]; dd = dd[:k]
        if len(idx) < 5: continue
        w = 1.0/(dd+1e-3); Xn = CXY[idx, 0]; Yn = CXY[idx, 1]; qx, qy = q[j]
        for fi in range(6):
            vals = CEE[idx, fi]; ok = np.isfinite(vals)
            if ok.sum() < 5: continue
            xx = Xn[ok]; yy = Yn[ok]; zz = vals[ok]; ww = w[ok]; mx = xx.mean(); my = yy.mean()
            A = np.column_stack([xx-mx, yy-my, np.ones(len(xx))]); AT = A.T*ww
            try:
                coef = np.linalg.solve(AT@A + 1e-3*np.eye(3), AT@zz); Ep[j, fi] = coef[0]*(qx-mx)+coef[1]*(qy-my)+coef[2]
            except Exception:
...
def well_feats(wid, split='train'):
    ...
    pf3_pred = pf_ensemble(h, tw, n_seeds=PF_SEEDS, MOM=0.99, VN=0.006, PN=0.012)
    st_pred, cov, st_kres, disp = struct_pred(h, exclude=wid if split == 'train' else None)
    st_plane_pred, st_plane_kres = struct_plane_pred(h, exclude=wid if split == 'train' else None)
    ...
    f = pd.DataFrame({
        ...
        'stp_dis': st_plane_pred[ev]-st_pred[ev], 'stp_pf_dis': st_plane_pred[ev]-pf_pred[ev],
