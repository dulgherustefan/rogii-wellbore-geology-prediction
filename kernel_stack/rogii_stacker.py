...
def main():
    ...
    tr = [x for x in Parallel(n_jobs=NJOBS, backend=BACKEND, verbose=5)(delayed(well_feats)(w, 'train') for w in train_wids) if x is not None]
    ...
    te = [x for x in Parallel(n_jobs=NJOBS, backend=BACKEND, verbose=5)(delayed(well_feats)(w, 'test') for w in test_wids) if x is not None]
    out = {}
    for fte in te:
