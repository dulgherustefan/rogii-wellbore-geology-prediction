...
OUT = Path('models_B15'); OUT.mkdir(exist_ok=True)
t0 = time.time()
...
CFGS = [
    ...
    ('lgbH', lambda: LGBMRegressor(**{**LGB, 'objective': 'huber'}, num_leaves=255,
        min_child_samples=25, subsample=0.8, subsample_freq=1, colsample_bytree=0.7,
        reg_lambda=10.0, learning_rate=0.03, n_estimators=5000, seed=11), True),
    ('lgbR', lambda: LGBMRegressor(**LGB, num_leaves=127, min_child_samples=200,
        subsample=0.7, subsample_freq=1, colsample_bytree=0.5, reg_lambda=200.0,
        reg_alpha=20.0, learning_rate=0.02, n_estimators=5000, seed=5), True),
]
...
np.save('meta_oof_b15.npy', meta)
np.save('oof_cols_b15.npy', OOFM)
